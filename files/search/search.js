...
(function (mod) {
  ...
  function getDoReplaceConfirm(cm) {
    return '<button><i class="fad fa-check"></i></button> <button><i class="fad fa-level-down"></i></button> <button><i class="fad fa-check-double"></i></button> <button><i class="fad fa-times"></i></button>';
  }

  function replaceAll(cm, query, text) {
    var state = getSearchState(cm);
    cm.removeOverlay(state.overlay, queryCaseInsensitive(query));
    if (cm.showMatchesOnScrollbar) {
      if (state.annotate) {
        state.annotate.clear();
        state.annotate = null;
        ...
        dialog(
          ...
          function (text) {
            ...
            if (all) {
              ...
              var state = getSearchState(cm);
              cm.removeOverlay(state.overlay, queryCaseInsensitive(query));
              state.overlay = searchOverlay(query, queryCaseInsensitive(query));
              cm.addOverlay(state.overlay);
              if (cm.showMatchesOnScrollbar) {
                if (state.annotate) {
                  state.annotate.clear();
